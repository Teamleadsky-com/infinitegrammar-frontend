...
// ── 9. App shell served to APP_FUNCTIONAL routes is noindex ──────────
// netlify.toml rewrites /exercise, /auth, ... to /app-shell.html. Those routes
// are crawlable (not in robots.txt), so this static noindex is the only thing
// keeping them out of the index (INDEX-015).

{
  const appShellPath = path.join(DIST, 'app-shell.html');
  const errors = [];
  if (!fs.existsSync(appShellPath)) {
    errors.push('dist/app-shell.html not found (generated by scripts/prerender.js)');
  } else {
    const html = fs.readFileSync(appShellPath, 'utf-8');
    if (!/<meta\s+name="robots"\s+content="noindex"/i.test(html)) {
      errors.push('app-shell.html: missing <meta name="robots" content="noindex">');
    }
    if (/rel="canonical"/i.test(html)) {
      errors.push('app-shell.html: must not declare a canonical');
    }
    if (/property="og:url"/i.test(html)) {
      errors.push('app-shell.html: must not declare og:url');
    }
  }
  if (errors.length === 0) pass('App shell is noindex with no canonical');
  else fail('App shell indexing errors', errors);
}
